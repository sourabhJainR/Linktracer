...
function applyLibraryView(view=viewValue()){
  ...
  if(smart)document.querySelectorAll('[data-smart-section]').forEach(section=>{section.hidden=value!=='all'&&section.dataset.smartSection!==value});
}
...
function wireLibraryControls(){
  ...
  if(!header)return;
  let views=document.getElementById('libraryViewNav')||document.getElementById('libraryViewTabs');
  if(!views){
    views=document.createElement('nav');
    views.id='libraryViewNav';
    views.className='libraryViewNav libraryViewTabs';
    views.setAttribute('role','tablist');
    views.setAttribute('aria-label','Library views');
    views.innerHTML='<button type="button" role="tab" data-library-view="all">All links</button><button type="button" role="tab" data-library-view="recent">Recent</button><button type="button" role="tab" data-library-view="favorites">Favorites</button><button type="button" role="tab" data-library-view="followups">Follow-up</button>';
    header.appendChild(views);
  }
  if(!views.__libraryViewWired){
  ...
  applyLibraryView();applyLayout();
}
function wireImportFocus(){
  const capture=document.getElementById('captureForm');
  if(capture?.onsubmit&&!capture.onsubmit.__focusWrapped){const previous=capture.onsubmit;const wrapped=async function(event){try{await previous.call(this,event);if(document.getElementById('url')?.value==='')focusAfterImport(this)}catch(error){throw error}};wrapped.__focusWrapped=true;capture.onsubmit=wrapped}
  const whatsapp=document.getElementById('whatsappImportBtn');
  if(whatsapp?.onclick&&!whatsapp.onclick.__focusWrapped){const previous=whatsapp.onclick;const wrapped=async function(event){const before=document.getElementById('whatsappBulkPreview')?.textContent||'';try{await previous.call(this,event);const after=document.getElementById('whatsappBulkPreview')?.textContent||'';if(/Imported \d+ new links/i.test(after)&&after!==before)focusAfterImport(this)}catch(error){throw error}};wrapped.__focusWrapped=true;whatsapp.onclick=wrapped}
  const file=document.getElementById('importFile');
  if(file?.onchange&&!file.onchange.__focusWrapped){const previous=file.onchange;const wrapped=async function(event){try{await previous.call(this,event);const status=document.getElementById('status')?.textContent||'';if(/^Imported /i.test(status))focusAfterImport(this)}catch(error){throw error}};wrapped.__focusWrapped=true;file.onchange=wrapped}
}
function loadStyles(){
...
.libraryViewNav,.libraryViewTabs{display:flex;gap:3px;padding:3px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:11px;overflow:auto}
.libraryViewNav button,.libraryViewTabs button{border:0;border-radius:8px;background:transparent;color:#64748b;min-height:32px;padding:6px 11px;font-size:11px;font-weight:750;white-space:nowrap;box-shadow:none}
.libraryViewNav button:hover,.libraryViewTabs button:hover{background:#e2e8f0;transform:none;box-shadow:none}
.libraryViewNav button.active,.libraryViewTabs button.active{background:#fff;color:#1e40af;box-shadow:0 2px 7px rgba(15,23,42,.10)}
.libraryViewNav button:focus-visible,.libraryViewTabs button:focus-visible{outline:2px solid #60a5fa;outline-offset:-2px}
...
.libraryGrid{display:grid!important;grid-template-columns:repeat(auto-fill,minmax(310px,1fr))!important;align-items:stretch}
#links.libraryGrid .link{min-width:0}
#links.libraryList{display:grid!important;grid-template-columns:1fr!important;gap:8px!important}
...
@media(max-width:900px){.libraryHeader{grid-template-columns:1fr auto}.libraryViewNav,.libraryViewTabs{grid-column:1/-1;grid-row:2}.layoutToggle{grid-column:2;grid-row:1}}
@media(max-width:700px){.libraryHeader{align-items:center}.layoutToggle button{padding-inline:8px}.libraryViewNav,.libraryViewTabs{width:100%}#links.libraryList .link{grid-template-columns:1fr;grid-template-areas:"main" "description" "badges" "tags" "actions"}#links.libraryList .linkActions{min-width:0;flex-direction:row;align-items:center;justify-content:space-between}#links.libraryList .linkActions>div{justify-content:flex-end}}
`;
  document.head.appendChild(style);
}
function init(){loadStyles();wireLibraryControls();wireImportFocus();document.addEventListener('linktracer-rendered',()=>{wireLibraryControls();applyLayout();applyLibraryView()},{passive:true});const observer=new MutationObserver(()=>{wireImportFocus();applyLibraryView()});observer.observe(document.body,{childList:true,subtree:true})}
